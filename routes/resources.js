...
async function handleResources(req, res) {
  ...
  if (path === '/api/resources' && method === 'POST') {
    ...
    try {
      ...
      if (categorySettings.enable_low_stock_threshold && parseFloat(newResource.quantity) < parseFloat(newResource.low_stock_threshold)) {
  ...
  if (path.startsWith('/api/resources/') && method === 'PUT' && path.split('/').length === 4) {
    ...
    try {
      ...
      if (updateResult.rows.length === 0) {
        ...
        if (categorySettings.enable_low_stock_threshold && parseFloat(currentNewQty) < parseFloat(currentThreshold) && parseFloat(currentNewQty) !== parseFloat(oldQty)) {
