...
async function checkResourceExpirations() {
    ...
    // const today = new Date(); // Original line, today.setHours was for the old calculation
    // today.setHours(0, 0, 0, 0); // NO LONGER NEEDED for daysUntilExpiration calculation
    ...
    try {
        ...
        for (const resource of resourceResult.rows) {
            ...
            // expiration_date is now a 'YYYY-MM-DD' string from the database
            const parts = resource.expiration_date.split('-');
            const year = parseInt(parts[0], 10);
            const month = parseInt(parts[1], 10) - 1; // Month is 0-indexed in JS Date
            const day = parseInt(parts[2], 10);
            
            const expDateUtcMidnight = new Date(Date.UTC(year, month, day)); // UTC midnight of expiration day

            // Get current local time to derive current day's UTC midnight
            ...
            // This difference is between two UTC midnights, so it will be a multiple of 24 hours.
            // The result will be an integer representing the number of full calendar days.
            ...
            for (const interval of NOTIFICATION_INTERVALS_DAYS) {
                if (daysUntilExpiration === interval) {
                    ...
                    // Check if a notification of this type for this resource was already sent today
                    const alreadyNotifiedToday = await pool.query(
                        ...
                        [resource.id, notificationType, new Date()] // Using new Date() for current timestamp is correct here
                    );
...
function startExpirationChecker(intervalMinutes = 60 * 24) { // Default to once a day (24 * 60 minutes)
